<template>
    ...
    <header class="shadow-xs bg-white ff-header" ref="ffHeader">
        ...
        <div
            ...
            class="fixed w-4 h-4 bg-[#FFDB1F] rounded-full pointer-events-none z-50 fly-to-cart-animation"
            :style="{
                left: flyingDot.x + 'px',
                top: flyingDot.y + 'px',
                '--target-x': flyingDot.targetX - flyingDot.x + 'px',
                '--target-y': flyingDot.targetY - flyingDot.y + 'px',
            }"
            @animationend="onFlyingDotEnd"
        ></div>
        ...
        >
            ...
            <nav class="items-center justify-center gap-6 hidden lg:flex">
                ...
                <router-link
                    ...
                    :class="
                        checkIsPathAndRoutePathSame('/menu')
                            ? 'text-primary'
                            : ''
                    "
                    class="capitalize text-sm font-normal text-heading-light tracking-[1px]"
            ...
            >
                ...
                >
                    <button
                        type="submit"
                        class="header-search-submit flex justify-center items-center pl-1 text-heading-light"
                    >
                        <i
                            class="lab lab-search-normal !text-sm text-heading-light"
                        ></i>
                    </button>
                    <input
                        type="search"
                ...
                <div v-else class="dropdown-group">
                    ...
                    >
                        ...
                        <i
                            class="lab lab-arrow-down-2 text-xs ml-1.5 lab-font-size-15"
                        ></i>
                    </button>
                    <div
                    ...
                    >
                        <div class="flex items-center gap-3 p-4 mb-2">
                            ...
                            <figure>
                                <figcaption class="flex-auto">
                                    <h3
                                        class="text-sm font-medium capitalize mb-0.5"
                                    >
                                        {{ profile.name }}
                                    </h3>
                        ...
                        <nav class="px-4">
                            ...
                            >
                                <i
                                    class="lab lab-reserve-line lab-font-size-17"
                                ></i>
                                <span class="text-sm leading-6 capitalize">{{
                                    $t("button.my_orders")
                                }}</span>
                            ...
                            >
                                <i
                                    class="lab lab-messages-line lab-font-size-17"
                                ></i>
                                <span class="text-sm leading-6 capitalize">{{
                                    $t("button.chat")
                                }}</span>
...
export default {
            ...
            .then((res) => {
                ...
                window.setTimeout(() => {
                    ...
                    this.$store.dispatch("frontendCart/initOrderType", {
                        order_setup_delivery:
                            res.data.data.order_setup_delivery,
                        order_setup_takeaway:
    ...
    methods: {
        ...
        search: function () {
            if (
                typeof this.searchItem !== "undefined" &&
                this.searchItem !== ""
            ) {
                this.$router.push({
        ...
        saveImage: function () {
            if (this.$refs.imageProperty.files[0]) {
                try {
                    ...
                    this.$store
                        .dispatch("frontendEditProfile/changeImage", {
                            form: formData,
                        })
                        .then((res) => {
                            this.$store
        ...
        triggerCartAnimation: function (buttonElement) {
            // Get the position of the add to cart button
            ...
            if (cartButton) {
                ...
                this.flyingDot.x = buttonRect.left + buttonRect.width / 2 - 8; // 8px for half dot width
                this.flyingDot.y = buttonRect.top + buttonRect.height / 2 - 8; // 8px for half dot height
                this.flyingDot.targetX = cartRect.left + cartRect.width / 2 - 8;
