<template>
    <div class="w-full max-w-[630px] rounded-2xl bg-white">
        ...
        <div
            v-if="setting.autocomplete || setting.currentLocation"
        ...
        <div
            ref="theGoogleMap"
            id="the-google-map"
            class="w-full h-[180px] rounded-xl mb-4"
        ></div>
    </div>
</template>
...
export default {
    ...
    mounted: async function () {
        ...
        if (cachedAddress && cachedCoordinates) {
            this.address = cachedAddress;
            const coords = JSON.parse(cachedCoordinates);
    ...
    methods: {
        ...
        setPosition: function () {
            ...
            const latLngLiteral = new google.maps.LatLng(
                this.currentLocation.lat,
                this.currentLocation.lng
            );
            const geocoder = new google.maps.Geocoder();
            geocoder
                .geocode({ latLng: latLngLiteral })
                .then((res) => {
                    for (let i = 0; i < res.results.length; i++) {
                        for (
                            let j = 0;
                            j < res.results[i].address_components.length;
                            j++
                        ) {
                            if (
                                res.results[i].address_components[j]
                                    .types[0] === "route" &&
                                other.rodeNo === null
                            ) {
                                other.rodeNo =
                            ...
                            for (
                                let k = 0;
                                k <
                                res.results[i].address_components[j].types
                                    .length;
                                k++
                            ) {
                                if (
                    ...
                    if (
                        other.country &&
                        other.country.toLowerCase() !== "nigeria"
                    ) {
                        alert("Only locations in Nigeria are allowed.");
                        this.address = "";
                        this.position({
                            address: "",
                            other: null,
