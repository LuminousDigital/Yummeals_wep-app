<template>
  <section class="mb-16 mt-12">
    <div class="container">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 lg:gap-12 mb-12 pt-4">
        <div class="space-y-8">
          <div>
            <h1 class="text-[18px] md:text-2xl lg:text-3xl font-bold text-[rgb(242,91,10)] mb-4">
              Refer Your Friends and Win
            </h1>
            <p class="text-[12px] md:text-[15px] lg:text-[17px] text-black font-medium leading-snug">
              Refer your friends and get amazing rewards <br />
              when they join us using your referral code.
            </p>
          </div>
            <p class="text-[13px] md:text-sm lg:text-base text-gray-700 font-medium mb-2">
              Referral Balance: <span class="text-green-600 font-semibold">{{ referralBalance }}</span>
            </p>
          <div>
            <h2 class="text-[12px] md:text-[15px] lg:text-[17px] text-black font-medium pb-2">
              Referral Link
            </h2>
            <div class="p-3 md:p-4">
              <p class="text-[14px] md:text-base text-[rgb(242,91,10)] font-medium break-all">
                {{ referralLink }}
              </p>
            </div>
            <div class="flex flex-wrap justify-center items-center gap-5 md:gap-6 lg:gap-8 mb-12 mt-10 w-full transition-all">
              <a
        ...
        <div class="bg-white rounded-lg shadow-sm px-4 sm:px-6">
          <h2 class="text-sm md:text-lg lg:text-xl font-semibold text-gray-900 mb-4 md:mb-6">
            Referral History ({{ referralHistory.length }})
          </h2>
          <div class="bg-white rounded-lg shadow-sm px-2 sm:px-4 md:px-6 h-auto sm:max-h-[350px] lg:max-h-[450px] overflow-y-auto hide-scrollbar relative">
            <div v-if="referralHistory.length === 0" class="w-full text-center py-10 flex flex-col items-center justify-center">
              <img
                src="/images/social-icon/empty-referrals.png"
                alt="No referrals"
                class="w-40 h-40 object-contain mb-4"
              />
              <p class="text-orange-600 text-sm md:text-base font-semibold">
            ...
            >
              <div>
                <h3 class="text-sm md:text-base lg:text-lg font-medium text-orange-500">
                  {{ referral.name }}
                </h3>
  ...
  methods: {
    ...
    async fetchReferralData() {
      try {
        ...
        this.referralHistory = data.referrals.map((item, index) => ({
          id: index + 1,
          name: item.name ?? item.email ?? "Unknown",
          date: new Date(item.created_at).toLocaleDateString("en-NG"),
          reward: item.reward ?? "NGN 500",
        }));
      } catch (error) {
