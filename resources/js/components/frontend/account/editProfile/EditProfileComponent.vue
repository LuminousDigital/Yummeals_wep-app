    ...
    <section class="pt-32 pb-16">
        <div class="container max-w-[550px]">
            ...
            <div class="py-6 p-4 sm:px-6 shadow-xs rounded-2xl bg-white">
                <h2 class="capitalize mb-6 text-left text-[22px] font-semibold leading-[34px] text-heading-light">
                    {{ $t('menu.edit_profile') }}
                </h2>
                <form @submit.prevent="save" id="formElem">
                    <div class="row">
                        <div class="col-12 sm:col-6">
                            <label for="first_name" class="text-xs capitalize mb-1 text-heading-light">{{ $t('label.first_name') }}</label>
                            <input id="first_name" type="text" v-model="form.first_name" :class="errors.first_name ? 'invalid' : ''"
                                class="w-full h-12 text-sm rounded-lg border px-4 text-heading-light border-[#D9DBE9]">
                            <small class="db-field-alert" v-if="errors.first_name">
                                {{ errors.first_name[0] }}
                            </small>
                        </div>
                        <div class="col-12 sm:col-6">
                            <label for="last_name" class="text-xs capitalize mb-1 text-heading-light">{{ $t('label.last_name') }}</label>
                        ...
                        <div class="col-12 sm:col-6">
                            <label for="email" class="text-xs capitalize mb-1 text-heading-light">{{ $t('label.email') }}</label>
                            <input disabled id="email" type="email" v-model="form.email" :class="errors.email ? 'invalid' : ''"
                                class="w-full h-12 text-sm rounded-lg border px-4 text-heading-light border-[#D9DBE9]">
                        ...
                        <div class="col-12 sm:col-6">
                            <label for="phone" class="text-xs capitalize mb-1 text-heading-light">{{ $t('label.phone') }}</label>
